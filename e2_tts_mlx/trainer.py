...
import mlx.nn as nn
from mlx.optimizers import Adam, linear_schedule, cosine_decay, join_schedules, clip_grad_norm
...
class E2Trainer:
    ...
    def save_checkpoint(self, step, finetune=False):
        mx.save_safetensors(f"e2tts_{step}", dict(tree_flatten(self.model.trainable_parameters())))
    ...
    def train(self, train_dataset, learning_rate, epochs, batch_size, log_every = 10, plot_every = 500, save_every=1000, checkpoint: int | None = None):
        num_samples = 3_000 # len(train_dataset)
        total_steps = num_samples * epochs
        decay_steps = total_steps - self.num_warmup_steps
        
        warmup_scheduler = linear_schedule(
            init=1e-8, end=learning_rate, steps=self.num_warmup_steps,
        )
        decay_scheduler = cosine_decay(
            init=learning_rate, decay_steps=decay_steps
        )
        ...
        def train_step(mel_spec, text_inputs, mel_lens):
            ...
            (loss, cond, pred, flow, w, mask), grads = loss_and_grad_fn(self.model, mel_spec, text=text_inputs, lens=mel_lens)
        ...
        for epoch in range(epochs):
            ...
            for batch in batched_dataset:
                ...
                text_inputs = [bytes(batch["transcript"][i]).decode("utf-8") for i in range(effective_batch_size)]
                
                mel_spec = rearrange(mx.array(batch["mel_spec"]), "b 1 n c -> b n c")
                mel_lens = mx.array(batch["mel_len"], dtype=mx.int32)
                
                (loss, cond, pred, flow, w, mask) = train_step(mel_spec, text_inputs, mel_lens)
                ...
                if global_step > 0 and global_step % log_every == 0:
                    ...
                    print(f"step {global_step}: loss = {loss.item():.4f}, sec per step = {elapsed_time / log_every}")
